...
export const AuthenticatedNavbar = ({
  ...
  const isAnnouncementRoute = announcementActivePaths.includes(location.pathname);
  const isRecordsRoute = recordsActivePaths.includes(location.pathname);
  const isPartnershipRoute = location.pathname.startsWith("/partnership&events");
  ...
  return (
    <header className={`bg-(--navbar-bg) px-6 py-4 ${sticky ? "sticky top-0 z-50" : ""}`}>
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-8">
          ...
          >
            <div className="relative">
              <button
                ...
                className={`cursor-pointer text-gray-900 transition-colors hover:text-gray-700 ${
                  openDropdown === "about" ? "text-xl font-bold" : "text-lg font-medium"
                }`}
                onClick={() =>
            ...
            <Link
              className={`cursor-pointer text-gray-900 transition-colors hover:text-gray-700 ${
                isAnnouncementRoute ? "text-xl font-bold" : "text-lg font-medium"
              }`}
              to={announcementPath}
            >
              Announcements
